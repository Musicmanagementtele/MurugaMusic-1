...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/3f0c65d3899e9985eb202.jpg"
            ...
            [
               ...
               [
                    InlineKeyboardButton(
                            text="ᴜᴘᴅᴀᴛᴇs",
                            url=f"https://t.me/throwpathidpworld"),
        ...
        if len(message.command) < 2:
            await message.reply_photo(
                     photo=f"https://telegra.ph/file/3f0c65d3899e9985eb202.jpg",
                    caption="💌 **ᴜsᴀɢᴇ: /play ɢɪᴠᴇ ᴀ ᴛɪᴛʟᴇ sᴏɴɢ ᴛᴏ ᴘʟᴀʏ ᴍᴜsɪᴄ**"
                    ,
                      reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("👥 sᴜᴘᴘᴏʀᴛ", url="https://t.me/tamilbestfriendss"),
                            InlineKeyboardButton("📢 ᴜᴘᴅᴀᴛᴇs", url="https://t.me/throwpathidpworld")
                        ]
                    ]
                )
            )
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.pause_stream(chat_id)
            await query.edit_message_text(
                "⏸ sikirama resume pannunga ☺️☺️", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **ᴇʀʀᴏʀ:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ sikirama resume pannunga ☺️☺️", show_alert=True)
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "▶️ Thank you song play pannathuku 😍😍", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **ᴇʀʀᴏʀ:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Thank you song play pannathuku 😍😍", show_alert=True)
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await query.edit_message_text("✅ **Y entha song pikalaya 😒😔**", reply_markup=bcl)
        except Exception as e:
            await query.edit_message_text(f"🚫 **ᴇʀʀᴏʀ:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Y entha song pikalaya 😒😔", show_alert=True)
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "🔇 Yaru entha song Mute Pannadhu 😒😒", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **ᴇʀʀᴏʀ:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Y entha song Mute Pannadhu 😒😒", show_alert=True)
...
async def cbunmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "🔊 Thank You Song Unmute Pannathuku 😍😍", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **ᴇʀʀᴏʀ:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Thank You Song Unmute Pannathuku 😍😍", show_alert=True)
